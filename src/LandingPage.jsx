...
function Icon({ name, className = "h-5 w-5" }) {
  ...
  switch (name) {
    ...
    case "palette":
      return <svg {...common}><path d="M12 3a9 9 0 0 0 0 18h1.5a1.5 1.5 0 0 0 0-3H12a2 2 0 0 1 0-4h3a6 6 0 0 0 0-12h-3z" /><circle cx="7.5" cy="10" r=".6" fill="currentColor" stroke="none" /><circle cx="10" cy="7" r=".6" fill="currentColor" stroke="none" /><circle cx="14" cy="7" r=".6" fill="currentColor" stroke="none" /></svg>;
    case "phone":
    ...
    case "sparkles":
      return <svg {...common}><path d="m12 3 1.5 4.5L18 9l-4.5 1.5L12 15l-1.5-4.5L6 9l4.5-1.5L12 3z" /><path d="m19 14 .8 2.2L22 17l-2.2.8L19 20l-.8-2.2L16 17l2.2-.8L19 14z" /><path d="m5 14 .8 2.2L8 17l-2.2.8L5 20l-.8-2.2L2 17l2.2-.8L5 14z" /></svg>;
    default:
...
function runContentSelfTests(content) {
  const errors = [];
  if (content.services.items.length < 1) errors.push("services powinno zawierac przynajmniej 1 element");
  if (content.benefits.items.length < 4) errors.push("benefits powinno zawierac przynajmniej 4 elementy");
  if (content.process.items.length < 2) errors.push("process powinno miec przynajmniej 2 kroki");
  if (!content.packages.items.some((pack) => pack.highlighted)) errors.push("jeden pakiet powinien byc wyrozniony");
...
function SectionTitle({ eyebrow, title, text }) {
  return (
    <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.35 }} variants={stagger} className="mx-auto mb-12 max-w-3xl text-center">
      <motion.p variants={fadeUp} className="mb-3 text-sm font-semibold uppercase tracking-[0.26em] text-cyan-300">{eyebrow}</motion.p>
      <motion.h2 variants={fadeUp} className="text-3xl font-black tracking-tight text-white md:text-5xl">{title}</motion.h2>
      {text && <motion.p variants={fadeUp} className="mt-5 text-lg leading-8 text-slate-300">{text}</motion.p>}
    </motion.div>
  );
}
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 md:px-10">
        ...
        <nav className="hidden items-center gap-8 text-sm text-slate-300 md:flex">
          {settings.navItems.map((item) => (
            <a key={`${item.label}-${item.href}`} href={item.href} onClick={(event) => scrollToHref(event, item.href)} className="transition hover:text-cyan-300">
              {item.label}
            </a>
          ))}
        </nav>
      ...
      {isOpen && (
        <div className="border-t border-white/10 bg-[#050816]/95 px-6 py-4 md:hidden">
          <nav className="flex flex-col gap-3 text-sm text-slate-200">
            {settings.navItems.map((item) => (
              <a key={`${item.label}-${item.href}`} href={item.href} onClick={(event) => scrollToHref(event, item.href)} className="rounded-lg bg-white/5 px-4 py-3 transition hover:bg-white/10">
                {item.label}
              </a>
            ))}
          </nav>
...
function NetworkMonitorLogo() {
  return (
    <motion.div animate={{ y: [0, -10, 0] }} transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }} className="relative mx-auto w-full max-w-[540px] rounded-lg border border-cyan-400/35 bg-slate-950/65 p-4 shadow-2xl shadow-blue-500/20 backdrop-blur">
      ...
      <div className="relative aspect-[16/10] rounded-lg border-2 border-transparent bg-slate-950 p-7 [background:linear-gradient(#050816,#050816)_padding-box,linear-gradient(135deg,#1d9bff,#9a4dff)_border-box]">
        <svg viewBox="0 0 420 250" className="h-full w-full overflow-visible" role="img" aria-label="Animowane logo monitora z globem sieciowym">
          ...
          <motion.ellipse cx="210" cy="124" rx="146" ry="48" fill="none" stroke="url(#globeGradient)" strokeWidth="9" strokeLinecap="round" filter="url(#glow)" animate={{ rotate: [0, 360] }} transition={{ duration: 18, repeat: Infinity, ease: "linear" }} style={{ transformOrigin: "210px 124px" }} />
          <motion.g animate={{ rotate: [0, -360] }} transition={{ duration: 34, repeat: Infinity, ease: "linear" }} style={{ transformOrigin: "210px 124px" }}>
            <circle cx="210" cy="124" r="90" fill="none" stroke="url(#globeGradient)" strokeWidth="4" />
            <ellipse cx="210" cy="124" rx="38" ry="90" fill="none" stroke="url(#globeGradient)" strokeWidth="2" opacity="0.9" />
            <ellipse cx="210" cy="124" rx="72" ry="90" fill="none" stroke="url(#globeGradient)" strokeWidth="2" opacity="0.65" />
            <path d="M120 124H300" stroke="url(#globeGradient)" strokeWidth="2" opacity="0.9" />
            <path d="M132 82C180 102 240 102 288 82" stroke="url(#globeGradient)" strokeWidth="2" opacity="0.65" />
            <path d="M132 166C180 146 240 146 288 166" stroke="url(#globeGradient)" strokeWidth="2" opacity="0.65" />
            {[[120, 124, 8], [150, 88, 6], [168, 158, 7], [210, 124, 7], [274, 92, 7], [284, 154, 6], [302, 124, 5]].map(([cx, cy, r], index) => (
              <motion.circle key={index} cx={cx} cy={cy} r={r} fill="url(#globeGradient)" filter="url(#glow)" animate={{ scale: [1, 1.25, 1], opacity: [0.7, 1, 0.7] }} transition={{ duration: 2.2, repeat: Infinity, delay: index * 0.18 }} />
            ))}
          </motion.g>
        </svg>
      </div>
      <div className="mx-auto h-8 w-24 bg-gradient-to-r from-blue-500 to-violet-500" style={{ clipPath: "polygon(25% 0, 75% 0, 92% 100%, 8% 100%)" }} />
      ...
      <div className="relative z-10 mx-auto grid max-w-7xl items-center gap-14 md:grid-cols-[1.05fr_0.95fr]">
        <div>
          ...
          <div className="mt-10 grid max-w-xl gap-3 sm:grid-cols-3">
            {hero.stats.map((stat) => (
              <div key={`${stat.value}-${stat.label}`} className="rounded-lg border border-white/10 bg-white/[0.045] p-4">
...
function ServicesSection({ services }) {
  return (
    <section id="oferta" className="relative overflow-hidden px-6 py-20 md:px-10 lg:py-24 scroll-mt-24">
      <div className="pointer-events-none absolute right-2 top-8 z-0 hidden opacity-55 md:block"><AnimatedCircuit variant="longDrop" className="h-28 w-80" /></div>
      <div className="relative z-10 mx-auto max-w-7xl">
        <SectionTitle eyebrow={services.eyebrow} title={services.title} text={services.text} />
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} variants={stagger} className="grid gap-5 md:grid-cols-3">
          {services.items.map((service) => (
            <motion.div key={service.title} variants={fadeUp} whileHover={{ y: -6 }} className="group rounded-lg border border-white/10 bg-white/[0.045] p-7 backdrop-blur transition hover:-translate-y-1 hover:border-cyan-300/40 hover:bg-white/[0.07] hover:shadow-2xl hover:shadow-cyan-500/10">
  ...
  return (
    <section id="korzysci" className="relative overflow-hidden px-6 py-20 md:px-10 lg:py-24 scroll-mt-24">
      <div className="relative z-10 mx-auto grid max-w-7xl gap-10 rounded-lg border border-white/10 bg-white/[0.045] p-7 backdrop-blur md:grid-cols-[0.85fr_1.15fr] md:p-10 card">
        <div>
          <p className="mb-3 text-sm font-semibold uppercase tracking-[0.26em] text-cyan-300">{benefits.eyebrow}</p>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {benefits.items.map((item) => (
            <div key={item} className="flex gap-3 rounded-lg border border-white/10 bg-slate-950/35 p-4">
        ...
        <div className="relative mx-auto max-w-4xl">
          ...
          <div className="space-y-5">
            {process.items.map((item) => (
              <div key={item.step} className="relative rounded-lg border border-white/10 bg-white/[0.045] p-6 backdrop-blur transition hover:border-violet-300/35 hover:shadow-2xl hover:shadow-violet-500/10 md:ml-16">
                <div className="absolute -left-[4.55rem] top-6 hidden h-12 w-12 items-center justify-center rounded-lg bg-gradient-to-r from-blue-500 to-violet-500 text-sm font-black text-white shadow-lg shadow-blue-500/30 md:flex">{item.step}</div>
      ...
      <div className="relative z-10 mx-auto max-w-7xl">
        ...
        <div className="grid gap-5 md:grid-cols-3">
          {portfolio.items.map((item, index) => (
            <article key={item.title} className="overflow-hidden rounded-lg border border-white/10 bg-white/[0.045] backdrop-blur transition hover:border-cyan-300/35 hover:shadow-2xl hover:shadow-cyan-500/10">
      ...
      <div className="relative z-10 mx-auto max-w-7xl">
        ...
        <div className="grid gap-5 md:grid-cols-3">
          {packages.items.map((pack) => (
            <div key={pack.name} className={`relative rounded-lg border p-7 backdrop-blur transition hover:-translate-y-1 hover:shadow-2xl ${pack.highlighted ? "border-cyan-300/40 bg-gradient-to-br from-blue-500/18 to-violet-500/18 shadow-2xl shadow-blue-500/10 hover:shadow-blue-500/15" : "border-white/10 bg-white/[0.045] hover:border-violet-300/35 hover:shadow-violet-500/10"}`}>
              {pack.highlighted && <div className="absolute -top-4 left-7 rounded-full bg-gradient-to-r from-blue-500 to-violet-500 px-4 py-1.5 text-xs font-bold text-white">Najczesciej wybierane</div>}
      ...
      <div className="relative z-10 mx-auto max-w-4xl">
        ...
        <div className="space-y-4">
          {faq.items.map((item, index) => (
            <div key={item.question} className="rounded-lg border border-white/10 bg-white/[0.045] backdrop-blur">
    ...
    <section id="kontakt" className="relative overflow-hidden px-6 py-20 md:px-10 lg:pb-24 lg:pt-20 scroll-mt-24">
      <div className="relative z-10 mx-auto max-w-7xl overflow-hidden rounded-lg border border-white/10 bg-white/[0.045] p-7 backdrop-blur md:p-10">
        <div className="grid gap-10 md:grid-cols-[1fr_0.85fr] md:items-center">
          <div>
            <p className="mb-3 text-sm font-semibold uppercase tracking-[0.26em] text-cyan-300">{contact.eyebrow}</p>
...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    loadPublishedSiteContent().then((result) => {
      ...
      setCmsWarning(result.usedFallback ? "CMS jest niedostepny lub nieskonfigurowany. Strona uzywa tresci domyslnej." : "");
    });
    return () => {
      cancelled = true;
    };
    ...
    <div className="relative min-h-screen overflow-x-hidden bg-[#050816] text-white">
      ...
      {(selfTestErrors.length > 0 || cmsWarning) && (
        <div className="fixed bottom-4 left-4 z-50 max-w-sm rounded-lg border border-amber-400/40 bg-amber-950/90 p-4 text-sm text-amber-100 shadow-2xl">
          ...
          {selfTestErrors.length > 0 && <ul className="mt-2 list-disc pl-5">{selfTestErrors.map((error) => <li key={error}>{error}</li>)}</ul>}
        </div>
      )}
